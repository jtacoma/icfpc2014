...
func (b *Block) PrependDbug(id int) int {
	b.Commands = append(
		[]Command{
			{"LDC", []interface{}{id}, "block number"},
			{"DBUG", nil, ""},
		},
		b.Commands...)
	b.Commands = append(b.Commands[:len(b.Commands)-1],
		Command{"LDC", []interface{}{id + 10000}, "block number"},
		Command{"DBUG", nil, ""},
		b.Commands[len(b.Commands)-1])
	for _, child := range b.Children {
		id = child.PrependDbug(id + 1)
	}
	return id + 1
}
